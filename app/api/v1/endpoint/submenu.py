...
def create_submenu(
        target_menu_id: str,
        submenu: schemas.CreateSubmenuSchema,
...
def update_submenu(
        ...
        submenu: schemas.UpdateMenuSchema,
        db: Session = Depends(get_db),
):
    ...
    submenu_query.update(
        submenu.model_dump(exclude_unset=True), synchronize_session=False
    )
    db.commit()
...
@router.delete(
    "/{target_menu_id}/submenus/{target_submenu_id}", status_code=status.HTTP_200_OK
)
